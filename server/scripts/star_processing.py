...
def build_time_range(start, end):
    ...
    while start <end:
        ...
        dates.append((start, start + timedelta(month_end,0,-1)))
        start += timedelta(month_end)
    return dates
...
cursor.execute("DROP TABLE IF EXISTS star_stats_retroactive")
query ="CREATE TABLE star_stats_retroactive(Id INT PRIMARY KEY AUTO_INCREMENT, enter_incident INT, exit_incident INT, total_use INT, start_date DATETIME, end_date DATETIME, user_name VARCHAR(64))"
cursor.execute(query)
...
for date in date_list:
    ...
    for user in users:
        ...
        query = "INSERT INTO star_stats_retroactive(enter_incident, exit_incident, total_use, start_date, end_date, user_name) VALUES({0}, {1}, {2},'{3}','{4}','{5}')".format(missed_enter, missed_exit, total, datetime.strftime(date[0], time_format), datetime.strftime(date[1], time_format), user[0])
        cursor.execute(query)
        db.commit()
